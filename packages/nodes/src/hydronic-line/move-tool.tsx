...
/** The hydronic line's footprint radius (meters) — half the diameter, used
 *  as box / footprint padding and ghost radius. */
function hydronicLineRadiusM(hydronicLine: HydronicLineNode): number {
  return (hydronicLine.diameter * IN_TO_M) / 2
}

/** XZ bounds of a path padded by the hydronic line's radius. */
...
/**
 * Ghost-preview duplicate / move tool for hydronic lines — the fluid-loop
 * sibling of `MoveLinesetTool`. A hydronic line is a single insulated tube;
 * the ghost stands in with a translucent cylinder at the tube diameter per
 * section (mirrors the draw tool's `PreviewSegment`).
 *
 ...
 */
export const MoveHydronicLineTool: React.FC<{ node: AnyNode }> = ({ node }) => {
  const hydronicLine = node as HydronicLineNode
  const originalPathRef = useRef<Vec3[]>(hydronicLine.path.map((p) => [...p] as Vec3))
  ...
  useEffect(() => {
    ...
    const r = hydronicLineRadiusM(hydronicLine)
    ...
    const commit = (event: GridEvent) => {
      ...
      if (isNew && !useScene.getState().nodes[nodeId]) {
        const created = HydronicLineNode.parse({
  ...
  return (
    <group>
      ...
      <DragBoundingBox
        ...
        size={[box.maxX - box.minX, hydronicLine.diameter * IN_TO_M, box.maxZ - box.minZ]}
      />
    </group>
  )
}

/** Translucent stand-in for one hydronic-line section — mirrors the draw
 *  tool's `PreviewSegment` so the ghost matches what actually lands. */
...
export default MoveHydronicLineTool
