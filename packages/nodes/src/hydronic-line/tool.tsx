...
import { emitter, type GridEvent, HydronicLineNode, useScene } from '@pascal-app/core'
import {
  CursorSphere,
  DimensionPill,
  EDITOR_LAYER,
...
import { collectScenePorts, findNearestPortXZ, HYDRONIC_PORT_SYSTEMS } from '../shared/ports'
import { hydronicLineDefinition } from './definition'

/**
 * Continuous placement tool for hydronic lines — the fluid-loop sibling of
 * the refrigerant lineset tool.
 *
 * Mouse-driven model:
 *   - **First click** anchors the run start. Within range of a hydronic
 *     service port (a boiler / manifold valve, or another hydronic line's
 *     end) it snaps onto the port so a run mates flush.
 *   - **Second click** commits a two-point hydronic line and keeps its far
 *     end anchored, so the next click continues the run like wall / duct
 *     drafting.
 *   - The in-flight end follows the active snapping mode: `angles` locks it
 *     to the nearest 45° step in XZ from the start (Y stays at the start's
 ...
 *
 * Snapping is restricted to hydronic ports, so a hydronic line never grabs a
 * supply/return duct collar.
 */
const PREVIEW_OPACITY = 0.6
const PREVIEW_COLOR = '#b45309'
/** Snap radius (meters) for joining onto a hydronic port. */
...
/** Nearest hydronic port within snap range on the XZ plane, as a
...
function findNearbyPort(point: [number, number, number]): [number, number, number] | null {
  const port = findNearestPortXZ(
    point,
    collectScenePorts({ systems: HYDRONIC_PORT_SYSTEMS }),
...
const HydronicLineTool = () => {
  ...
  useEffect(() => {
    ...
    const commitSegment = (start: [number, number, number], end: [number, number, number]) => {
      ...
      // Each drawn segment is its own standalone two-point hydronic-line node
      // — the fluid-loop sibling of duct-segment. Independent nodes mean each
      // segment selects and deletes on its own, rather than folding into one
      // mitered polyline run.
      const hydronicLine = HydronicLineNode.parse({
        ...hydronicLineDefinition.defaults(),
        name: 'Hydronic line',
        path: [start, end],
      })
      useScene.getState().createNode(hydronicLine, activeLevelId)
    ...
    // Figma-style alignment so a hydronic line lines up with other runs,
    // equipment, and items as it's drawn. A free point (first vertex, or no
    // angle lock) snaps; an angle-locked continuation shows the guide
    // passively so it doesn't fight the angle ray. Alignment follows the
    // `lines` mode; a port snap or Alt-vertical bypasses it.
  ...
  return (
    <LevelOffsetGroup>
      {/* Cursor marker — the same ground ring + vertical line + tool-icon
          ...
          `hydronic-line` structure-tools entry). In 2D the floorplan overlay
...
function PreviewSegment({ a, b }: { a: [number, number, number]; b: [number, number, number] }) {
  ...
  // Default diameter (1/2") for the ghost.
  const radius = (0.5 * 0.0254) / 2
  return (
    <mesh
      layers={EDITOR_LAYER}
...
export default HydronicLineTool
